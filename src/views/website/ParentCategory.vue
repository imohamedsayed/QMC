    ...
    <div class="category py-10">
        <v-container>
            ...
            <div v-else-if="!loading && category">
                ...
                <v-row>
                    <v-col cols="6" md="4" lg="2" v-for="cat in childCategories" :key="cat.id">
                        <v-list-item
                            :title="cat.name"
                            :prepend-avatar="apiUrl + cat.cover"
                            :to="'/category/' + id + '/' + cat.id"
                            color="black"
                            rounded="lg"
                            class="bg-grey100 font-weight-bold"
                            elevation="1"
                        >
                        </v-list-item>
                    </v-col>
                </v-row>
                <div class="product-container mt-10">
...
<script setup>
import Products from '@/components/website/Category/ParentProducts.vue';
...
const loadCategory = async () => {
    ...
    try {
        const res = await axios.get(`api/products/${props.id}/parent?latcreatedAt=desc`);
        if (res.status == 200) {
            pagination.value = res.data.meta.pagination;
            category.value = res.data.parentCategory;
            products.value = res.data.products;
        } else {
            throw new Error(res.response?.data?.message || 'Something went wrong, try again later');
        }

        const catsRes = await axios.get(`api/categories/child/${props.id}`);
        if (catsRes.status == 200) {
            childCategories.value = catsRes.data.categories;
        } else {
            throw new Error(catsRes.response?.data?.message || 'Something went wrong, try again later');
        }
    } catch (err) {
