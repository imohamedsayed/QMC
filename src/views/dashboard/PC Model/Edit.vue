<template>
    <v-row>
        <v-col cols="12" md="12">
            <UiParentCard title="Edit a PC Model" :loading="state.loading">
                <div class="pa-10">
...
export default {
    ...
    setup(props) {
        ...
        onMounted(async () => {
            ...
            try {
                const modelRes = await axios.get('api_dashboard/pc-models/' + props.id);
                if (modelRes.status == 200) {
                    const model = modelRes.data.data;
                    state.pathImage = modelRes.data.path_image;
                    state.name = model.name;
                    state.description = model.description;
                    state.price = model.price;
                    state.quantity = model.quantity;
                    state.status = model.status == 'Enable';
                    state.color = model.color;
                    state.os = model.Operating_system;
                    state.weight = model.Weight_in_gram;
                    state.usesFor = model.uses_for;
                    state.brand = model.brand.id;
                    state.pc = model.pc.id;
                    state.storedMedia = model.media.map((m) => m.name);
                } else {
                    throw new Error(modelRes.response.data.message);
                }
            } catch (error) {
                toast.error(error.message);
            }
        ...
        const edit = async () => {
            ...
            if (!v$.value.$error) {
                ...
                try {
                    ...
                    const res = await axios.post('api_dashboard/pc-models/' + props.id, data, {
                        headers: {
                    ...
                    if (res.status == 202) {
                        toast.success('PC Model updated Successfully');
                    } else {
                        throw new Error(res.response.data.message);
                    }
                } catch (err) {
        ...
        return { state, getImageUrl, v$, edit };
    }
};
</script>
