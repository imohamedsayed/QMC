<template>
    <v-row>
        <v-col cols="12" md="12">
            <UiParentCard title="Add a new why-us box" :loading="state.loading">
                <div class="pa-10">
        ...
        const add = async () => {
            ...
            if (!v$.value.$error) {
                ...
                try {
                    ...
                    const res = await axios.post('api_dashboard/whyus', data);

                    if (res.status == 201) {
                        toast.success('Why-us Box Added Successfully');
                    } else {
