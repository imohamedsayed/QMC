<template>
    <v-row>
        <v-col cols="12" md="12">
            <UiParentCard title="Edit FAQ" :loading="state.loading">
                <div class="pa-10">
...
export default {
    ...
    setup(props) {
        ...
        onMounted(async () => {
            ...
            state.loading = true;
            try {
                const res = await axios.get('api_dashboard/faqs/' + props.id);
                if (res.status == 200) {
                    const faq = res.data.data;
                    state.name_ar = faq.question_ar;
                    state.name_en = faq.question_en;
                    state.description_ar = faq.answer_ar;
                    state.description_en = faq.answer_en;
                    state.status = Boolean(Number(faq.status));
                } else {
        ...
        const edit = async () => {
            ...
            if (!v$.value.$error) {
                ...
                try {
                    ...
                    const res = await axios.post('api_dashboard/faqs/' + props.id, data);
