    ...
    <div class="pa-8 pb-16">
        ...
        >
            ...
            <template #item-action="item">
                <v-btn icon elevation="0" class="mr-2" @click="$router.push({ name: 'EditWhyUs', params: { id: item.id } })"
                    ><v-icon color="primary">mdi-pencil</v-icon>
                    <v-tooltip activator="parent">Edit why us box</v-tooltip>
                </v-btn>
        ...
        <v-dialog v-model="dialog" width="500" transition="dialog-top-transition" persistent>
            <v-card class="pa-4" :loading="loading">
                <v-card-title>Do you really want to delete this whyUs box?</v-card-title>
...
const headers = ref([
    ...
    { text: 'Title (En)', value: 'name_en' },
    { text: 'Title (Ar)', value: 'name_ar' },
...
const loadItems = async () => {
    loading.value = true;
    try {
        const res = await axios.get('api_dashboard/whyus?page=' + serverOptions.value.page);
        if (res.status === 200) {
            console.log(res);
...
const confirmDeleteItem = async () => {
    loading.value = true;
    try {
        const res = await axios.delete('api_dashboard/whyus/' + id.value);
        if (res.status == 204) {
            toast.success('WhyUs Box Deleted Successfully', { autoClose: 1000 });
            dialog.value = false;
            loadItems();
        } else {
            throw new Error(res.response.data.message);
