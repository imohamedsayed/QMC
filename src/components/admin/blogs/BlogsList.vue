    ...
    <div class="pa-8 pb-16">
        ...
        >
            ...
            <template #item-action="item">
                <v-btn icon elevation="0" class="mr-2" @click="$router.push({ name: 'EditBlog', params: { id: item.id } })"
        ...
        <v-dialog v-model="dialog" width="500" transition="dialog-top-transition" persistent>
            <v-card class="pa-4" :loading="loading">
                <v-card-title>Do you really want to delete this blog?</v-card-title>
                <v-card-text>After confirming the deletion of this blog you cannot undo the changes you made</v-card-text>
...
const loadItems = async () => {
    loading.value = true;
    try {
        const res = await axios.get('api_dashboard/blogs?page=' + serverOptions.value.page);
        if (res.status === 200) {
...
const confirmDeleteItem = async () => {
    loading.value = true;
    try {
        const res = await axios.delete('api_dashboard/blogs/' + id.value);
        if (res.status == 204) {
